public class App {
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("-------------------------------");
        Warlock Saruman = new Warlock("Saruman The Warlock", 50, 150, 150);
        Saruman.Display(); // -- Display Warlock Stats --//
        System.out.println("-------------------------------"); // -- First Round --//
        System.out.println("Round One");
        Merlin.fireStorm(Saruman); // -- Merlin attack Saruman --//
        Merlin.mpCost(Merlin); // -- Merlin reduce MP --//
        Saruman.infernalPunishment(Merlin); // -- Saruman attack Merlin --//
        Saruman.mpCost(Saruman); // -- Saruman reduce MP --//
        System.out.println("-------------------------------"); // -- Second Round --//
        System.out.println("Second Round");
        Merlin.HealingGaling(Merlin); // -- Merlin Use Recover HP MP skill --//
        Saruman.heavensTreatment(Saruman); // -- Saruman Use Recover HP MP skill --//
        System.out.println("-------------------------------"); // -- Third Round --//
        System.out.println("Third Round");
        Merlin.GuardiansWraith(Saruman); // -- Merlin attack Saruman --//
        Merlin.mpCost(Merlin); // -- Merlin reduce MP --//
        Saruman.meteorSmash(Merlin); // -- Saruman attack Merlin --//
        Saruman.mpCost(Saruman); // -- Saruman reduce MP --//
        System.out.println("-------------------------------"); // -- Fourth Round --//
        System.out.println("Final Round");
        Merlin.lightningStrike(Saruman); // -- Merlin attack Saruman --//
        Merlin.mpCost(Merlin); // - Merlin reduce MP --//
        System.out.println("-------------------------------"); // -- Game OVer --//
        Merlin.WoL(Saruman); // -- If statement for Win or Lose --//
    }
}
